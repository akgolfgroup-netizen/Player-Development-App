  ...
  return (
    <div className="flex flex-col h-[calc(100vh-160px)] bg-ak-surface-base">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <div className="hidden w-80 flex-none border-l border-ak-border-default px-8 py-6 md:block">
          <CalendarMiniMonth
          ...
          <section className="mt-10">
            <h2 className="text-base font-semibold text-ak-text-primary">
              Kommende økter
            </h2>
            <ol className="mt-4 space-y-1 text-sm leading-6 text-ak-text-secondary">
              {enhancedEvents
                ...
                .map((event) => (
                  <li
                    ...
                    className="group flex items-center gap-x-4 rounded-xl px-4 py-2 cursor-pointer hover:bg-ak-surface-subtle transition-colors"
                  >
                    <div className={clsx(
                    ...
                    <div className="flex-auto">
                      <p className="text-ak-text-primary font-medium truncate">{event.title}</p>
                      <p className="mt-0.5 flex items-center gap-1 text-xs text-ak-text-tertiary">
                        <Clock size={12} />
              ...
              {enhancedEvents.filter((e) => e.date >= new Date().toISOString().split('T')[0]).length === 0 && (
                <li className="text-ak-text-tertiary py-2">Ingen kommende økter</li>
              )}
            </ol>
          </section>
        </div>
      ...
      <div className="max-w-2xl mx-auto">
        {sortedEvents.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-ak-text-tertiary">Ingen økter denne dagen</p>
          </div>
        ) : (
          <ol className="space-y-3">
            {sortedEvents.map((event) => (
              ...
              >
                <div className="flex-none w-16 text-sm font-medium text-ak-text-secondary">
                  {event.start}
                </div>
                <div className="flex-1">
                  <p className="font-semibold text-ak-text-primary">{event.title}</p>
                  {event.location && (
                    <p className="text-sm text-ak-text-tertiary">{event.location}</p>
                  )}
                </div>
                <div className="flex-none text-sm text-ak-text-tertiary">
                  {event.start} - {event.end}
                </div>
              </li>
            ))}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 max-w-4xl mx-auto">
        {months.map((month) => (
          <button
            ...
            className="p-4 rounded-lg bg-ak-surface-base border border-ak-border-default hover:border-ak-primary hover:bg-ak-surface-subtle transition-colors text-left"
          >
            <p className="font-semibold text-ak-text-primary">{month.name}</p>
            {month.eventCount > 0 && (
              <p className="text-sm text-ak-text-tertiary mt-1">
