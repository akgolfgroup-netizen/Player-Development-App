  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="sticky top-0 z-30 flex-none bg-ak-surface-base shadow-sm ring-1 ring-black/5 dark:ring-white/10 sm:pr-8">
          ...
          {weekDates.map((date, idx) => {
            ...
            return (
              <button key={dateKey} type="button" className="flex flex-col items-center pt-2 pb-3">
                <span>{WEEKDAYS_SHORT[idx].charAt(0)}</span>
                <span
                  className={clsx(
        ...
        {/* Desktop day header */}
        <div className="-mr-px hidden grid-cols-7 divide-x divide-ak-border-subtle border-r border-ak-border-subtle text-sm leading-6 text-ak-text-tertiary sm:grid">
          <div className="col-end-1 w-14" />
          {weekDates.map((date, idx) => {
            const dateKey = formatDateKey(date);
            const isToday = dateKey === todayKey;
            return (
              <div key={dateKey} className="flex items-center justify-center py-3">
                <span className={isToday ? 'flex items-baseline' : ''}>
                  ...
                  <span
                    className={clsx(
                      ...
                      isToday
                        ? 'ml-1.5 flex h-8 w-8 rounded-full bg-ak-primary text-white'
                        : 'text-ak-text-primary'
                    )}
                  >
                    {date.getDate()}
