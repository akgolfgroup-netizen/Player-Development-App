...
const ViewToggle = ({ currentView, onChange }) => {
  const views = [
    ...
    { id: VIEW_MODES.TOURNAMENT, label: 'Turnering' },
  ];
...
const AKGolfKalender = ({ events = [] }) => {
  ...
  const getNavigationTitle = () => {
    switch (viewMode) {
      ...
      case VIEW_MODES.TOURNAMENT:
        return `Turneringer ${currentDate.getFullYear()}`;
      default:
        return '';
    }
  };
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: tokens.colors.snow }}>
      <PageHeader
        title="Kalender"
        subtitle="Treningsplan og aktiviteter"
        actions={
          <button
      ...
      <div style={{ padding: '24px', maxWidth: '900px', margin: '0 auto' }}>
        ...
        {viewMode === VIEW_MODES.TOURNAMENT && (
          <TournamentView
            currentYear={currentDate.getFullYear()}
            tournaments={[]}
            onTournamentClick={(tournament) => {
              navigate(`/turneringer/${tournament.id}`);
            }}
            onNavigate={navigate_calendar}
          />
        )}
