  ...
  return (
    <div className="min-h-screen bg-ak-surface-subtle">
      <div className="flex h-screen">
        {/* Sidebar */}
        <aside className="hidden lg:flex lg:w-80 lg:flex-col lg:border-r lg:border-ak-border-default lg:bg-ak-surface-base">
          {/* Mini Calendar */}
          ...
          <div className="flex-1 overflow-y-auto">
            <div className="p-4">
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-semibold text-ak-text-primary flex items-center gap-2">
                  <Bell className="h-4 w-4" />
                  Booking-forespørsler
                  {bookingRequests.length > 0 && (
                    <span className="px-1.5 py-0.5 bg-ak-status-warning text-white text-xs font-semibold rounded-full">
                ...
                <button
                  ...
                  className="text-xs text-ak-primary hover:text-ak-primary/80"
                >
                  Se alle
                </button>
              </div>
              ...
              {bookingRequests.length === 0 ? (
                <p className="text-sm text-ak-text-tertiary text-center py-4">
                ...
                <ul className="space-y-2">
                  {bookingRequests.slice(0, 3).map((request) => (
                    <li
                      key={request.id}
                      className="p-3 bg-ak-surface-subtle rounded-lg border border-ak-status-warning/30"
                    >
                      <div className="flex items-start gap-2 mb-2">
                        <div className="w-8 h-8 rounded-full bg-ak-primary text-white flex items-center justify-center text-xs font-semibold flex-shrink-0">
                      ...
                      <div className="flex items-center gap-2 text-xs text-ak-text-secondary mb-2">
                      ...
                      {request.notes && (
                        <p className="text-xs text-ak-text-tertiary italic mb-2 truncate">
                          "{request.notes}"
                        </p>
                      )}
                      <div className="flex gap-2">
                        <button
                          ...
                          className="flex-1 py-1.5 text-xs font-medium text-ak-status-error border border-ak-status-error rounded hover:bg-ak-status-error/10 transition-colors"
                        >
                          Avslå
                        </button>
                        <button
                          onClick={() => handleApproveRequest(request.id)}
                          className="flex-1 py-1.5 text-xs font-medium text-white bg-ak-status-success rounded hover:bg-ak-status-success/90 transition-colors"
                        >
                          Godkjenn
                        </button>
                      </div>
            ...
            {/* Today's Sessions */}
            <div className="p-4 border-t border-ak-border-default">
              <h3 className="text-sm font-semibold text-ak-text-primary mb-3">
                I dag ({todaysSessions.length} økter)
              </h3>
              {todaysSessions.length === 0 ? (
                <p className="text-sm text-ak-text-tertiary text-center py-2">
                  Ingen økter i dag
                </p>
              ) : (
                <ul className="space-y-2">
                  {todaysSessions.map((session) => (
                    <li key={session.id}>
                      <button
                        ...
                        className="w-full p-2 rounded-lg bg-ak-primary/5 hover:bg-ak-primary/10 text-left transition-colors border-l-2 border-ak-primary"
                      >
            ...
            <div className="p-4 border-t border-ak-border-default">
              ...
              <div className="space-y-2">
                <button
                  ...
                  className="w-full flex items-center gap-2 p-2 rounded-lg text-sm text-ak-text-secondary hover:bg-ak-surface-subtle transition-colors"
                >
            ...
            {view === 'day' && (
              <div className="h-full p-6 overflow-y-auto">
                <div className="max-w-3xl mx-auto">
                  <h2 className="text-lg font-semibold text-ak-text-primary mb-4">
                  ...
                  {sessions.filter((s) => s.date === formatDateKey(currentDate)).length === 0 ? (
                    <div className="text-center py-12 text-ak-text-tertiary">
                      ...
                      {sessions
                        ...
                        .map((session) => (
                          <li key={session.id}>
                            <button
                              onClick={() => handleSessionClick(session)}
                              className="w-full p-4 rounded-xl bg-ak-surface-base border border-ak-border-default hover:border-ak-primary/50 text-left transition-colors"
                            >
                              <div className="flex items-start gap-3">
                                <div className="w-10 h-10 rounded-full bg-ak-primary text-white flex items-center justify-center text-sm font-semibold flex-shrink-0">
                                  {session.playerInitials}
                                </div>
                                <div className="flex-1 min-w-0">
                                  <p className="text-base font-semibold text-ak-text-primary">
                                    {session.playerName}
                                  </p>
                                  <p className="text-sm text-ak-text-secondary">
                                    {session.sessionType}
                                  </p>
                                  <div className="flex items-center gap-3 mt-2 text-sm text-ak-text-tertiary">
                                ...
                                <ChevronRight className="h-5 w-5 text-ak-text-tertiary" />
                              </div>
                            </button>
                          </li>
                        ))}
                  ...
                  {Array.from({ length: 12 }, (_, i) => {
                    ...
                    return (
                      <button
                        ...
                        className="p-4 rounded-xl bg-ak-surface-base border border-ak-border-default hover:border-ak-primary/50 text-left transition-colors"
                      >
                        <h3 className="text-sm font-semibold text-ak-text-primary capitalize">
                          {monthName}
                        </h3>
                        <p className="text-2xl font-bold text-ak-primary mt-1">
      ...
      {selectedSession && (
        <>
          ...
          />
          <div className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-ak-surface-base rounded-xl shadow-xl z-50 max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              {/* Header */}
              <div className="flex items-start justify-between mb-6">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-full bg-ak-primary text-white flex items-center justify-center text-lg font-semibold">
                ...
                >
                  <X className="h-5 w-5 text-ak-text-tertiary" />
                </button>
              </div>
              ...
              <div className="space-y-4 mb-6">
                <div className="flex items-center gap-3 p-3 bg-ak-surface-subtle rounded-lg">
                  <CalendarIcon className="h-5 w-5 text-ak-text-tertiary" />
                  <div>
                    <p className="text-xs text-ak-text-tertiary">Dato</p>
                    <p className="text-sm font-medium text-ak-text-primary">
                ...
                <div className="flex items-center gap-3 p-3 bg-ak-surface-subtle rounded-lg">
                  <Clock className="h-5 w-5 text-ak-text-tertiary" />
                  <div>
                    <p className="text-xs text-ak-text-tertiary">Tidspunkt</p>
                    <p className="text-sm font-medium text-ak-text-primary">
                ...
                {selectedSession.location && (
                  <div className="flex items-center gap-3 p-3 bg-ak-surface-subtle rounded-lg">
                    <MapPin className="h-5 w-5 text-ak-text-tertiary" />
                    <div>
                      <p className="text-xs text-ak-text-tertiary">Sted</p>
                      <p className="text-sm font-medium text-ak-text-primary">
                        {selectedSession.location}
                      </p>
                    </div>
                ...
                {selectedSession.notes && (
                  <div className="p-3 bg-ak-primary/5 rounded-lg border-l-2 border-ak-primary">
                    <p className="text-xs text-ak-text-tertiary mb-1">Notat</p>
                    <p className="text-sm text-ak-text-primary">{selectedSession.notes}</p>
                  </div>
                )}
              ...
              <div className="flex gap-3">
                <button
                  ...
                  className="flex-1 py-2.5 px-4 bg-ak-primary text-white rounded-lg text-sm font-semibold hover:bg-ak-primary/90 transition-colors"
                >
                  Se spillerprofil
                </button>
                {selectedSession.status !== 'completed' && (
                  <button
                    ...
                    className="p-2.5 rounded-lg border border-ak-status-success text-ak-status-success hover:bg-ak-status-success/10 transition-colors"
