...
const L_PHASES: Record<LPhase, { label: string; color: string }> = {
  'L-KROPP': { label: 'Kropp', color: 'rgb(var(--text-tertiary))' },
  'L-ARM': { label: 'Arm', color: 'rgb(var(--text-secondary))' },
  'L-KØLLE': { label: 'Kølle', color: 'rgb(var(--tier-navy))' },
  'L-BALL': { label: 'Ball', color: 'rgb(var(--tier-navy-dark))' },
  'L-AUTO': { label: 'Auto', color: 'rgb(var(--tier-navy))' },
};

const CATEGORY_GROUPS: Record<CategoryGroup, { label: string; color: string }> = {
  fullSwing: { label: 'Full Swing', color: 'rgb(var(--status-info))' },
  shortGame: { label: 'Naerspill', color: 'rgb(var(--status-success))' },
  putting: { label: 'Putting', color: 'rgb(var(--tier-gold))' },
};
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      ...
      <StandardPageHeader
        ...
        helpText="Analyser treningsfordelingen din på tvers av golfslag-typer (TEE, Innspill, Chip, Pitch, Putting), L-faser (Kropp→Arm→Kølle→Ball→Auto) og kategorier (Full Swing, Nærspill, Putting). Se timer, repetisjoner og fokusscore."
      />
