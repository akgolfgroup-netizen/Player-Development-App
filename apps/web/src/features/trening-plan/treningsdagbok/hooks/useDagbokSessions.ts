...
export function useDagbokSessions(state: DagbokState): UseDagbokSessionsResult {
  ...
  const queryParams = useMemo(() => {
    const params: Record<string, string> = {
      startDate: state.rangeStart.toISOString().split('T')[0],
      endDate: state.rangeEnd.toISOString().split('T')[0],
      includeDrills: 'true',
    };

    if (state.pyramid) params.pyramid = state.pyramid;
    if (state.area) params.area = state.area;
    if (state.lPhase) params.lPhase = state.lPhase;
    if (state.environment) params.environment = state.environment;
    if (state.pressure) params.pressure = state.pressure;
    if (state.csLevel !== null) params.csLevel = String(state.csLevel);
    if (state.planType !== 'all') params.planType = state.planType;
  ...
  useEffect(() => {
    const fetchSessions = async () => {
      ...
      try {
        ...
        const mockSessions = generateMockSessions(state);
