...
function AuthPageRoot({ children, state = 'idle', maxWidth = 'md' }: AuthPageProps) {
  return (
    <AuthPageContext.Provider value={{ state }}>
      <div className="min-h-screen flex items-center justify-center bg-ak-surface-base p-6">
...
function AuthPageLogo({ children, title, subtitle }: AuthPageLogoProps) {
  return (
    <div className="text-center mb-8">
      ...
      {title && (
        <h1 className="text-xl font-bold text-ak-text-primary mb-1">
          {title}
        </h1>
      )}
      {subtitle && (
...
function AuthPageCard({ children, title }: AuthPageCardProps) {
  return (
    <div className="bg-ak-surface-card rounded-2xl p-8 shadow-ak-elevated">
      {title && (
        <h2 className="text-xl font-bold text-ak-text-primary text-center mb-6">
          {title}
        </h2>
      )}
      {children}
...
function AuthPageFooter({ children }: AuthPageFooterProps) {
  return (
    <div className="mt-6 p-4 bg-ak-surface-card rounded-lg text-center">
