...
export const Avatar: React.FC<AvatarProps> = ({
  ...
  const resolvedBadgeColor = badgeColor ?? (
    status === "online" || status === "active" ? "var(--ak-status-success)" :
    status === "away" ? "var(--ak-status-warning)" :
    status === "offline" || status === "inactive" ? "var(--ak-text-muted)" :
    "var(--ak-action-active)"
  );
