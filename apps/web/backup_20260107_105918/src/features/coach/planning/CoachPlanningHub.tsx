...
const AthleteCard = React.memo(({ athlete, onClick }: { athlete: Athlete; onClick: () => void }) => {
  return (
    <div
      onClick={onClick}
      className="flex items-center gap-4 p-4 bg-tier-white rounded-xl border border-tier-border-default cursor-pointer hover:border-tier-navy transition-colors will-change-[border-color]"
      role="button"
      tabIndex={0}
        ...
        {isLoading ? (
          ...
          <>
            <div className="bg-tier-white rounded-xl border border-tier-border-default p-4">
              <div className="text-2xl font-bold text-tier-navy">{stats.athletes.withPlan}</div>
              <div className="text-sm text-tier-text-secondary">Spillere med plan</div>
            </div>
            <div className="bg-tier-white rounded-xl border border-tier-border-default p-4">
              <div className="text-2xl font-bold text-yellow-600">{stats.athletes.withoutPlan}</div>
              <div className="text-sm text-tier-text-secondary">Spillere uten plan</div>
            </div>
            <div className="bg-tier-white rounded-xl border border-tier-border-default p-4">
              <div className="text-2xl font-bold text-tier-navy">{stats.groups.withPlan}</div>
              <div className="text-sm text-tier-text-secondary">Grupper med plan</div>
            </div>
            <div className="bg-tier-white rounded-xl border border-tier-border-default p-4">
              <div className="text-2xl font-bold text-yellow-600">{stats.groups.withoutPlan}</div>
              <div className="text-sm text-tier-text-secondary">Grupper uten plan</div>
            </div>
          </>
        )}
      </div>
        ...
        <div className="space-y-3">
          {filteredGroups.length === 0 ? (
            <div className="text-center py-12 bg-tier-white rounded-xl border border-tier-border-default">
              ...
              <p className="text-tier-text-secondary">Ingen grupper funnet</p>
            </div>
          ) : (
