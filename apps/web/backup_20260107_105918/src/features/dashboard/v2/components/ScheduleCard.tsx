...
const ScheduleCard: React.FC<ScheduleCardProps> = ({
  ...
  const getTypeDot = (type: SessionType): string => {
    const colors: Record<SessionType, string> = {
      teknikk: 'rgb(var(--status-info))',
      golfslag: 'rgb(var(--status-success))',
      spill: 'rgb(var(--status-warning))',
      kompetanse: 'var(--tier-navy)',
      fysisk: 'rgb(var(--status-error))',
      funksjonell: 'rgb(var(--status-error))',
      hjemme: 'var(--gray-500)',
      test: 'rgb(var(--status-info))',
    };
    return colors[type];
  };
  ...
  const getStatusStyles = (status: SessionStatus): React.CSSProperties => {
    switch (status) {
      ...
      case 'in_progress':
        return {
          borderLeft: '3px solid var(--tier-navy)',
