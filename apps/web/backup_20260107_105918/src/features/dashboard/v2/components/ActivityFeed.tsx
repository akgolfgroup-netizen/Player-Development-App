...
const ActivityFeed: React.FC<ActivityFeedProps> = ({
  ...
  const getTypeIndicator = (type: ActivityType, isGold?: boolean): React.CSSProperties => {
    ...
    if (type === 'badge_earned' && isGold) {
      return { backgroundColor: 'var(--tier-gold)' };
    }

    const colors: Record<ActivityType, string> = {
      session_completed: 'rgb(var(--status-success))',
      badge_earned: 'var(--tier-navy)',
      goal_achieved: 'rgb(var(--status-success))',
      coach_message: 'rgb(var(--status-info))',
      test_result: 'var(--tier-navy)',
      streak_update: 'rgb(var(--status-warning))',
      system: 'var(--gray-400)',
    };
