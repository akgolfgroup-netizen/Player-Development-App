...
const SwimlaneFocusMatrix = ({ yearPlan, focusAreas, priorityLabels }) => {
  ...
  const getIntensityStyle = (value) => {
    const colors = {
      ...
      2: { bg: 'var(--warning)', opacity: 0.7 },
      3: { bg: 'var(--success)', opacity: 1 },
    };
          ...
          <div style={{ flex: 1, display: 'flex', gap: '2px' }}>
            {yearPlan.map((month, idx) => {
              ...
              return (
                ...
                >
                  {value > 0 && (
                    <span style={{ fontSize: '9px', fontWeight: 600, color: value >= 2 ? '#FFFFFF' : 'var(--text-secondary)' }}>
                      {value}
                    </span>
                  )}
                </div>
  ...
  return (
    <div id="aarsplan-export" style={{ minHeight: '100vh', backgroundColor: 'var(--bg-secondary)', fontFamily: 'Inter, -apple-system, system-ui, sans-serif' }}>
      ...
      <div style={{ padding: '24px', width: '100%' }}>
        {/* Player Summary Card */}
        <Card className="mb-6 border-0" style={{ background: 'linear-gradient(135deg, #245A85 0%, #1A5A8A 100%)' }}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <Avatar name={player.name} size={56} />
              <div>
                <SectionTitle className="text-[20px] font-bold" style={{ color: '#FFFFFF' }}>{player.name}</SectionTitle>
            ...
            <div className="flex gap-8">
              <div className="text-center">
                ...
                <p className="text-[24px] font-bold" style={{ color: '#FFFFFF' }}>{player.category}</p>
              </div>
              <div className="text-center">
                <p className="text-[11px] uppercase tracking-wide" style={{ color: 'rgba(255, 255, 255, 0.6)' }}>Snitt</p>
                <p className="text-[24px] font-bold" style={{ color: '#FFFFFF' }}>{player.avgScore}</p>
              </div>
              <div className="text-center">
                <p className="text-[11px] uppercase tracking-wide" style={{ color: 'rgba(255, 255, 255, 0.6)' }}>Målscore</p>
                <p className="text-[24px] font-bold" style={{ color: '#FFD700' }}>{player.targetScore}</p>
              </div>
            </div>
          </div>
        </Card>
        ...
        <div className="mb-6 flex flex-wrap items-center gap-4 p-3 bg-white rounded-xl border border-ak-mist">
          <span className="text-[11px] text-ak-steel font-medium uppercase tracking-wide">Perioder:</span>
          {Object.entries(periodConfig).map(([key, val]) => (
            <div key={key} className="flex items-center gap-1.5">
              <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: val.color }} />
              <span className="text-[12px] text-ak-charcoal">{val.name}</span>
            </div>
          ))}
          <div className="h-3 w-px bg-ak-mist" />
          <span className="text-[11px] text-ak-steel font-medium uppercase tracking-wide">Prioritet:</span>
          {[3, 2, 1].map(p => (
            <div key={p} className="flex items-center gap-1.5">
              <div className="w-2.5 h-2.5 rounded-sm" style={{ backgroundColor: priorityLabels[p].color }} />
              <span className="text-[12px] text-ak-charcoal">{priorityLabels[p].label}</span>
            </div>
          ))}
        </div>
                          ...
                          {focusAreas.map(area => (
                            <div key={area.key} className="flex items-center justify-between">
                              <div className="flex items-center gap-1.5">
                                <span className="text-ak-steel">{area.icon}</span>
                                <span className="text-[12px] text-ak-charcoal font-medium">{area.label}</span>
                              </div>
