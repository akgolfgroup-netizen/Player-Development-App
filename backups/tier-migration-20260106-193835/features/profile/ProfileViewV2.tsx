...
const getCategoryColor = (category?: string): string => {
  const colors: Record<string, string> = {
    'A': 'rgb(74, 144, 226)', // Blue
    'B': 'rgb(126, 211, 33)', // Green
    'C': 'rgb(245, 166, 35)', // Orange
  };
  return colors[category || ''] || 'rgb(155, 155, 155)';
};
  ...
  <div className="bg-white rounded-xl p-6 shadow-sm border border-gray-200 hover:shadow-md transition-all hover:-translate-y-1">
    <div className="flex items-center justify-between mb-2">
      <div
        ...
        style={{ backgroundColor: category ? getCategoryColor(category) + '20' : 'rgb(243, 244, 246)' }}
      >
        <div style={{ color: category ? getCategoryColor(category) : 'rgb(107, 114, 128)' }}>
          {icon}
        </div>
      </div>
    </div>
    <div className="text-3xl font-bold mb-1" style={{ color: category ? getCategoryColor(category) : 'rgb(45, 55, 72)' }}>
      {value || '-'}
    </div>
...
const ProfileSection: React.FC<ProfileSectionProps> = ({ icon, title, children, accentColor = 'rgb(74, 144, 226)' }) => (
