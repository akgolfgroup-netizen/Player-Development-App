...
export const CalendarWeekGrid: React.FC<CalendarWeekGridProps> = ({
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="sticky top-0 z-30 flex-none bg-ak-surface-base shadow-sm ring-1 ring-black/5 dark:ring-white/10 sm:pr-8">
        {/* Mobile day header */}
        <div className="grid grid-cols-7 text-sm leading-6 text-ak-text-tertiary sm:hidden">
            ...
            return (
              <button key={dateKey} type="button" className="flex flex-col items-center pt-2 pb-3">
                ...
                <div className="flex flex-col items-center">
                  ...
                  <span className="text-[10px] text-ak-text-secondary font-medium mt-0.5">
                    {monthAbbr}
                  </span>
                </div>
              </button>
        ...
        {/* Desktop day header */}
        <div className="-mr-px hidden grid-cols-7 divide-x divide-ak-border-subtle border-r border-ak-border-subtle leading-6 sm:grid">
          ...
          {weekDates.map((date, idx) => {
            ...
            return (
              <div key={dateKey} className="flex flex-col items-center justify-center py-3 px-2">
                <span className="text-xs text-ak-text-tertiary font-medium mb-1">
                  {WEEKDAYS_SHORT[idx]}
                </span>
                <div className="flex items-baseline gap-1">
                  <span
                    className={clsx(
                      ...
                      isToday
                        ? 'h-9 w-9 rounded-full bg-ak-primary text-white'
                        : 'text-ak-text-primary'
                    )}
                  >
                    {date.getDate()}
                  </span>
                  <span className="text-xs text-ak-text-secondary font-medium">
                    {monthAbbr}
                  </span>
                </div>
              </div>
      ...
      <div ref={scrollRef} className="flex flex-auto overflow-auto">
        {/* Time column */}
        <div className="sticky left-0 z-10 w-14 flex-none bg-ak-surface-base ring-1 ring-ak-border-subtle" />
        ...
        <div className="grid flex-auto grid-cols-1 grid-rows-1">
          ...
          >
            ...
            {timeSlots.map((time, idx) => (
              <React.Fragment key={time}>
                <div>
                  <div className="sticky left-0 z-20 -ml-14 -mt-2.5 w-14 pr-2 text-right text-xs leading-5 text-ak-text-tertiary">
                    {time}
                  </div>
                </div>
                <div />
          ...
          {/* Vertical day lines */}
          <div className="col-start-1 col-end-2 row-start-1 hidden grid-cols-7 grid-rows-1 divide-x divide-ak-border-subtle sm:grid">
            {weekDates.map((date) => (
                    ...
                    return group.map((event, groupIdx) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="hidden dark:block pointer-events-none absolute inset-1 z-0 rounded-lg bg-ak-surface-base" />
