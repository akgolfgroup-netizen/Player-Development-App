...
export const CalendarMiniMonth: React.FC<CalendarMiniMonthProps> = ({
  ...
  return (
    <div className={className}>
      ...
      {/* Weekday headers */}
      <div className="mt-6 grid grid-cols-7 text-xs leading-6 text-ak-text-tertiary text-center">
      ...
      {/* Days grid */}
      <div className="isolate mt-2 grid grid-cols-7 gap-px rounded-lg bg-ak-border-subtle text-sm shadow-sm ring-1 ring-ak-border-subtle overflow-hidden">
          ...
          <button
            ...
            className={clsx(
              ...
              dayInfo.isCurrentMonth
                ? 'bg-ak-surface-base hover:bg-ak-surface-subtle'
                : 'bg-ak-surface-subtle/50 hover:bg-ak-surface-subtle',
              // Corner rounding
              idx === 0 && 'rounded-tl-lg',
              idx === 6 && 'rounded-tr-lg',
          ...
          >
            <time
              ...
              className={clsx(
                ...
                // Text color
                !dayInfo.isSelected && !dayInfo.isToday && !dayInfo.isCurrentMonth && 'text-ak-text-tertiary',
                !dayInfo.isSelected && !dayInfo.isToday && dayInfo.isCurrentMonth && 'text-ak-text-primary',
                // Today (not selected)
                dayInfo.isToday && !dayInfo.isSelected && 'text-ak-primary font-semibold',
                // Selected (not today)
                dayInfo.isSelected && !dayInfo.isToday && 'bg-ak-text-primary text-white font-semibold',
                // Selected and today
                dayInfo.isSelected && dayInfo.isToday && 'bg-ak-primary text-white font-semibold',
              )}
            >
              {dayInfo.day}
            </time>
            ...
            {dayInfo.hasEvents && !dayInfo.isSelected && (
              <span className="absolute bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-ak-primary" />
            )}
          </button>
        ))}
      </div>
