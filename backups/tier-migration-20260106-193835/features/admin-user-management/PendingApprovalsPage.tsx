        ...
        <Page.Section>
          ...
          {users.length === 0 ? (
            <div className="text-center py-12">
              <Clock size={48} className="mx-auto mb-4 text-ak-text-tertiary" />
            ...
            <div className="space-y-4">
              {users.map((user) => (
                <div
                  key={user.id}
                  className="border border-ak-border-default rounded-lg p-4 bg-ak-surface-card"
                >
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        {user.status === 'approved' && (
                          <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-ak-status-success-light text-ak-status-success text-xs font-medium">
                            <CheckCircle size={14} />
                            Godkjent
                          </span>
                        )}
                        {user.status === 'rejected' && (
                          <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-ak-status-error-light text-ak-status-error text-xs font-medium">
                            <XCircle size={14} />
                            Avvist
                          </span>
                        )}
                        {user.status === 'pending' && (
                          <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-ak-status-warning-light text-ak-status-warning text-xs font-medium">
                            <Clock size={14} />
                            Venter
                          </span>
                        )}
                      ...
                      <div className="flex items-center gap-4 text-sm text-ak-text-tertiary">
