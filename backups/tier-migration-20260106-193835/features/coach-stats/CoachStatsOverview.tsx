...
export default function CoachStatsOverview() {
  ...
  const getCategoryColors = (category: string) => {
    switch (category) {
      ...
      default:
        return {
          bg: 'bg-ak-surface-muted',
          border: 'border-transparent',
          text: 'text-ak-primary',
          bgActive: 'bg-ak-primary/5',
        };
    }
  };
                ...
                className={`px-3 py-1.5 text-sm font-medium rounded-lg transition-all ${
                  isActive
                    ? colors
                      ...
                      : 'bg-ak-primary text-white border-2 border-ak-primary'
                    : colors
                    ? `${colors.bg} ${colors.text} border-2 border-transparent hover:${colors.border}`
                    : 'bg-ak-surface-muted text-ak-text-primary border-2 border-transparent hover:border-ak-border-default'
                }`}
              >
