...
export const CoachDataGolfStats: React.FC = () => {
  ...
  const formatSgValue = (value: number | null) => {
    if (value === null || value === undefined) return { text: 'N/A', color: 'text-ak-text-tertiary' };
  ...
  const getCategoryClasses = (category: string) => {
    switch (category) {
      ...
      default: return { bg: 'bg-ak-surface-muted', text: 'text-ak-text-secondary', border: 'border-ak-border-default' };
    }
  };
  ...
  return (
    <div className="p-6 bg-ak-surface-base min-h-screen">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div className="bg-ak-surface-base rounded-xl p-4 border border-ak-border-default">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm text-ak-text-secondary">Totalt spillere</span>
          </div>
          <p className="text-2xl font-bold text-ak-text-primary">{dashboardData.totalPlayers ?? 0}</p>
        </div>
        <div className="bg-ak-surface-base rounded-xl p-4 border border-ak-border-default">
          <div className="flex items-center gap-2 mb-2">
            <BarChart3 size={16} className="text-green-600" />
            <span className="text-sm text-ak-text-secondary">Med Data Golf data</span>
          </div>
          <p className="text-2xl font-bold text-ak-text-primary">{dashboardData.playersWithData ?? 0}</p>
        </div>
        <div className="bg-ak-surface-base rounded-xl p-4 border border-ak-border-default">
          <div className="flex items-center gap-2 mb-2">
            <Target size={16} className="text-orange-600" />
            <span className="text-sm text-ak-text-secondary">Gj.snitt SG Total</span>
          </div>
        ...
        <div className="bg-ak-surface-base rounded-xl p-4 border border-ak-border-default">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm text-ak-text-secondary">Tour</span>
          </div>
          <p className="text-xl font-bold text-ak-text-primary">{getTourDisplayName(tour)}</p>
        </div>
      </div>
      ...
      <div className="bg-ak-surface-base rounded-2xl p-5 mb-6 border border-ak-border-default">
        ...
        <div className="flex gap-4 mb-4">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full py-3 pr-3 pl-10 rounded-[10px] border border-ak-border-default bg-ak-surface-subtle text-sm text-ak-text-primary outline-none focus:border-ak-primary focus:ring-1 focus:ring-ak-primary"
            />
          </div>
        </div>
        ...
        <div className="mb-4">
          <p className="text-sm text-ak-text-secondary mb-2">Filtrer på kategori:</p>
                ...
                <button
                  ...
                  className={`
                    ...
                    ${isSelected
                      ...
                      : 'bg-ak-surface-subtle text-ak-text-primary border border-ak-border-default hover:bg-ak-surface-muted'
                    }
                  `}
        ...
        <div className="border-t border-ak-border-default pt-4">
          <div className="flex items-center justify-between mb-3">
            <div>
              <p className="text-sm font-medium text-ak-text-primary flex items-center gap-2">
        ...
        {showProComparison && (
          <div>
          <p className="text-xs text-ak-text-tertiary mb-3">
          ...
          <div className="relative max-w-md mb-3">
            ...
            <input
              ...
              className="w-full py-3 pr-10 pl-10 rounded-[10px] border border-ak-border-default bg-ak-surface-subtle text-sm text-ak-text-primary outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500"
            />
            {proPlayersLoading && (
          ...
          {proSearchQuery.length > 0 && proSearchQuery.length < 2 && (
            <p className="text-xs text-ak-text-tertiary mb-2">Skriv minst 2 tegn for å søke...</p>
          )}

          {/* Pro Player Selection */}
          <div className="flex flex-wrap gap-2 min-h-[44px]">
            {proPlayersLoading ? (
              <p className="text-sm text-ak-text-tertiary flex items-center gap-2">
                <Loader2 size={14} className="animate-spin" /> Søker i DataGolf...
              </p>
            ) : displayedProPlayers.length === 0 && proSearchQuery.length >= 2 ? (
              <p className="text-sm text-ak-text-tertiary">Ingen spillere funnet for "{proSearchQuery}"</p>
            ) : (
          ...
          {proSearchQuery.length === 0 && displayedProPlayers.length > 0 && (
            <p className="text-xs text-ak-text-tertiary mt-2">
        ...
        {filteredPlayers.length === 0 ? (
          ...
          <div className="space-y-3">
            {filteredPlayers.map((player) => (
              <div
                key={player.playerId}
                className="bg-ak-surface-subtle rounded-xl p-4 border border-ak-border-default"
              >
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-ak-text-secondary">HCP: {player.handicap?.toFixed(1) ?? 'N/A'}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-ak-text-tertiary mb-1">Total SG</p>
                ...
                <div className="grid grid-cols-4 gap-3">
                  {[
                    ...
                    <div key={idx} className="bg-ak-surface-base rounded-lg p-3">
                      <p className="text-[11px] text-ak-text-tertiary mb-1">{cat.label}</p>
                      <p className={`text-lg font-bold ${formatSgValue(cat.value).color}`}>
                        {formatSgValue(cat.value).text}
                      </p>
                      <p className="text-[10px] text-ak-text-tertiary mt-1">
                        {cat.percentile}%ile
                      </p>
                    </div>
                  ))}
