              ...
              return (
                <div key={key} className="border-b border-ak-surface-subtle">
                  ...
                  {isExpanded && daySchedule.enabled && (
                    <div className="py-4 px-5 bg-gradient-to-br from-ak-surface-base via-ak-surface-subtle to-ak-surface-subtle">
                      {daySchedule.slots.map((slot, idx) => (
                        <div
                          key={idx}
                          className="flex items-center gap-3 mb-3 p-3 bg-white rounded-xl border border-ak-border-default shadow-sm hover:shadow-md hover:border-ak-primary/30 transition-all"
                        >
                          <div className="w-10 h-10 rounded-lg bg-ak-primary/10 flex items-center justify-center shrink-0">
                            <Clock size={18} className="text-ak-primary" />
                          </div>
                          <div className="flex items-center gap-3 flex-1">
                            <div className="flex items-center gap-2">
                              <span className="text-[13px] font-medium text-ak-text-secondary">Fra</span>
                              <input
                                type="time"
                                value={slot.start}
                                onChange={(e) => updateTimeSlot(key, idx, 'start', e.target.value)}
                                className="py-2 px-3 border-2 border-ak-border-default rounded-lg text-sm text-ak-text-primary font-semibold focus:border-ak-primary focus:outline-none bg-ak-surface-base"
                              />
                            </div>
                            <div className="w-8 h-0.5 bg-ak-border-default rounded-full" />
                            <div className="flex items-center gap-2">
                              <span className="text-[13px] font-medium text-ak-text-secondary">til</span>
                              <input
                                type="time"
                                value={slot.end}
                          ...
                          {daySchedule.slots.length > 1 && (
                            <button
                              ...
                              className="w-9 h-9 rounded-lg bg-ak-status-error/10 border-none flex items-center justify-center cursor-pointer hover:bg-ak-status-error/20 transition-colors shrink-0"
                            >
              ...
              {settings.sessionTypes.map((session) => (
                <div
                  ...
                  className={`flex items-center justify-between p-4 px-5 rounded-xl border-2 shadow-sm hover:shadow-md transition-all cursor-pointer ${
                    session.enabled
                      ? 'bg-gradient-to-br from-ak-primary/10 via-ak-primary/5 to-transparent border-ak-primary/40 hover:border-ak-primary'
                      : 'bg-white border-ak-border-default hover:border-ak-border-default/80'
                  }`}
                ...
                >
                  <div className="flex items-center gap-4">
                    <button
                      ...
                      className={`w-7 h-7 rounded-lg flex items-center justify-center cursor-pointer border-2 transition-all shadow-sm ${
                        session.enabled
                          ? 'bg-ak-primary border-ak-primary scale-105'
                          : 'bg-ak-surface-base border-ak-border-default hover:border-ak-primary/30'
                      }`}
                    >
                      {session.enabled && (
                    ...
                    <div>
                      <p className={`text-[15px] font-semibold m-0 ${session.enabled ? 'text-ak-text-primary' : 'text-ak-text-secondary'}`}>
                        {session.name}
                      </p>
                      {session.description && (
                        <p className="text-xs text-ak-text-secondary mt-1 m-0">
                          {session.description}
                        </p>
                      )}
                    </div>
                  ...
                  <span className={`text-[13px] font-semibold py-1.5 px-3 rounded-full ${
                    session.enabled
                      ? 'text-ak-primary bg-white border border-ak-primary/20'
                      : 'text-ak-text-secondary bg-ak-surface-subtle'
                  }`}>
