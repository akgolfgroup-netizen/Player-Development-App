                ...
                {weekDates.map((date) => {
                  ...
                  return (
                    <div
                      ...
                      className={`p-2 min-h-[60px] ${statusClasses.bg} ${slot?.booking ? `border-l-[3px] ${statusClasses.border}` : ''} ${slot?.booking || slot?.status === 'available' ? 'cursor-pointer hover:bg-ak-primary/5' : 'cursor-default'} ${time === '12:00' ? 'border-t-2 border-t-ak-border-default' : ''}`}
                    >
                      {slot?.booking && (
                        <div>
