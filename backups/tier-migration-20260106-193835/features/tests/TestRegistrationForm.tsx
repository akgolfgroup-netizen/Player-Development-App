  ...
  return (
    <div
      className={`
        ...
        ${isActive ? 'border-ak-primary bg-ak-primary/5 shadow-md' : ''}
        ${isCompleted && !isActive ? 'border-ak-status-success/30 bg-ak-status-success/5' : ''}
        ${!isCompleted && !isActive ? 'border-ak-border-default bg-ak-surface-base' : ''}
      `}
    >
      {/* Shot number */}
      <div
        className={`
          ...
          ${isCompleted ? 'bg-ak-status-success text-white' : 'bg-ak-surface-muted text-ak-text-secondary'}
        `}
      >
      ...
      <div className="flex items-center gap-2">
        <input
          ...
          className={`
            ...
            focus:outline-none focus:ring-2 focus:ring-ak-primary/20 focus:border-ak-primary
            ${isActive ? 'border-ak-primary' : 'border-ak-border-default'}
          `}
        />
        <span className="text-sm text-ak-text-secondary min-w-[30px]">{unit}</span>
      </div>

      {/* Points (if applicable) */}
      {shot.poeng !== null && (
        <div className="text-right">
          <div className="text-lg font-bold text-ak-primary">{shot.poeng}</div>
          <div className="text-xs text-ak-text-secondary">poeng</div>
        </div>
      )}
    </div>
  );
...
export default function TestRegistrationForm({
  ...
  const renderInstructions = () => (
    <div className="space-y-6">
      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1">
          <h2 className="text-xl font-bold text-ak-text-primary">{test.navn}</h2>
          <p className="text-sm text-ak-text-secondary mt-1">{test.beskrivelse}</p>
        </div>
      </div>

      {/* Test details */}
      <div className="bg-ak-surface-muted rounded-xl p-5 space-y-4">
        <div className="grid grid-cols-2 gap-4">
          <div>
            <div className="text-xs font-medium text-ak-text-tertiary uppercase tracking-wider">
              Måling
            </div>
            <div className="text-sm font-medium text-ak-text-primary mt-1">
              {test.maling}
            </div>
          </div>
          <div>
      ...
      <button
        ...
        className="w-full py-4 bg-ak-primary text-white font-semibold rounded-xl text-lg hover:bg-ak-primary/90 transition-all flex items-center justify-center gap-2"
      >
        <Play size={20} />
        Start test
      </button>
  ...
  const renderRecording = () => (
    <div className="space-y-6">
      ...
      <div className="bg-ak-surface-muted rounded-xl p-4">
        <div className="text-sm text-ak-text-secondary mb-1">Aktivt slag</div>
      ...
      <div className="flex gap-3">
        <button
          ...
          className="flex-1 py-3 border border-ak-border-default rounded-xl font-medium text-ak-text-primary hover:bg-ak-surface-muted transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
          <ChevronLeft size={18} />
          Forrige
        </button>
        {activeShot < shots.length - 1 ? (
          <button
            onClick={nextShot}
            className="flex-1 py-3 bg-ak-primary text-white rounded-xl font-medium hover:bg-ak-primary/90 transition-all flex items-center justify-center gap-2"
          >
            Neste
          ...
          <button
            ...
            className="flex-1 py-3 bg-ak-status-success text-white rounded-xl font-medium hover:bg-ak-status-success/90 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
            Se resultat
            <Check size={18} />
          </button>
      ...
      <button
        onClick={resetShots}
        className="w-full py-2 text-ak-text-secondary hover:text-ak-status-error transition-all flex items-center justify-center gap-2 text-sm"
      >
        <RotateCcw size={16} />
        Nullstill alle slag
      </button>
  ...
  const renderReview = () => (
    <div className="space-y-6">
      {/* Results summary */}
      <div className="bg-ak-primary/5 border-2 border-ak-primary/20 rounded-xl p-6 text-center">
        <div className="text-sm text-ak-text-secondary mb-2">Ditt resultat</div>
        <div className="text-4xl font-bold text-ak-primary mb-2">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="bg-ak-surface-muted rounded-xl p-4 text-center">
          <div className="text-2xl font-bold text-ak-text-primary">{completedShots}</div>
          <div className="text-xs text-ak-text-secondary">Slag registrert</div>
        </div>
        <div className="bg-ak-surface-muted rounded-xl p-4 text-center">
          <div className="text-2xl font-bold text-ak-text-primary">
            {calculateBest(shots, test.maling === 'm').toFixed(1)}
          </div>
          <div className="text-xs text-ak-text-secondary">Beste slag</div>
        </div>
        <div className="bg-ak-surface-muted rounded-xl p-4 text-center">
          <div className="text-2xl font-bold text-ak-text-primary">{totalPoints}</div>
          <div className="text-xs text-ak-text-secondary">Total poeng</div>
        </div>
      </div>

      {/* All shots summary */}
      <div>
        <div className="text-sm font-medium text-ak-text-primary mb-3">Alle slag</div>
            ...
            <div
              ...
              className={`
                ...
                ${shot.resultatLengde !== null
                  ...
                  : 'bg-ak-surface-muted text-ak-text-secondary'
                }
              `}
            >
      ...
      <div>
        ...
        <textarea
          ...
          rows={3}
          className="w-full py-3 px-4 rounded-xl border border-ak-border-default text-sm resize-none focus:outline-none focus:ring-2 focus:ring-ak-primary/20 focus:border-ak-primary"
        />
      </div>

      {/* Error message */}
      {error && (
        <div className="flex items-center gap-2 p-4 bg-ak-status-error/10 text-ak-status-error rounded-xl">
          <AlertCircle size={18} />
      ...
      <div className="flex gap-3">
        <button
          onClick={() => setStep('recording')}
          className="flex-1 py-3 border border-ak-border-default rounded-xl font-medium text-ak-text-primary hover:bg-ak-surface-muted transition-all flex items-center justify-center gap-2"
        >
          <ChevronLeft size={18} />
          Rediger
        </button>
  ...
  return (
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
      <div className="bg-ak-surface-card rounded-2xl w-full max-w-lg max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-ak-border-subtle">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-ak-text-primary">{test.navn}</h3>
