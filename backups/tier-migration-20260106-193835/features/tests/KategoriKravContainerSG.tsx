  ...
  return (
    <Card variant="default" padding="md">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            ...
            {requirement.met ? (
              ...
              <Circle size={14} className="text-ak-text-tertiary" />
            )}
          </div>
          <CardTitle className="text-sm font-semibold text-ak-text-primary mb-1">
            {requirement.name}
          </CardTitle>
      ...
      <div className="flex justify-between items-center text-xs">
        <div>
          <div className="text-ak-text-tertiary">Krav</div>
          <div className="font-semibold text-ak-text-primary">
            {requirement.lowerIsBetter ? '≤' : '≥'} {requirement.requirement.toFixed(2)}{requirement.unit}
          </div>
        </div>
        <div className="text-center">
          <div className="text-ak-text-tertiary">Din</div>
          <div
        ...
        <div className="text-right">
          <div className="text-ak-text-tertiary">Diff</div>
          <div
            className={`font-semibold ${
              difference === null
                ? 'text-ak-text-tertiary'
      ...
      {requirement.sgValue !== undefined && (
        <div className="mt-2 pt-2 border-t border-ak-border-default">
          <div className="flex justify-between items-center text-xs">
            <span className="text-ak-text-secondary">Strokes Gained</span>
            <span
              className={`font-semibold ${
          ...
          {requirement.tourPercentile !== undefined && (
            <div className="flex justify-between items-center text-xs mt-1">
              <span className="text-ak-text-secondary">Tour Percentile</span>
              <span className="font-semibold text-ak-primary">
      ...
      {CATEGORY_SG_BENCHMARKS.slice().reverse().map((cat) => {
        ...
        return (
          <Button
            ...
            size="sm"
            className={`${
              isCurrent && !isSelected ? 'bg-ak-status-success/15 text-ak-status-success' : ''
            } ${isSelected ? 'border-2 border-ak-primary' : 'border-2 border-transparent'}`}
          >
            {cat.category}
            {isCurrent && ' ✓'}
          </Button>
  ...
  return (
    <Card variant="default" padding="md" className="mb-5">
      <div className="flex items-center gap-3 mb-4">
        <div className="w-12 h-12 rounded-full bg-ak-primary/15 flex items-center justify-center">
          <Award size={24} className="text-ak-primary" />
        </div>
        <div>
          <SectionTitle className="text-lg font-bold text-ak-text-primary m-0">
            Krav for Kategori {benchmark.category}
          </SectionTitle>
          <p className="text-[13px] text-ak-text-secondary mt-0.5">
      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="p-3 bg-ak-surface-subtle rounded-lg text-center">
          <div
            className={`text-2xl font-bold ${
              meetsCount === totalCount ? 'text-ak-status-success' : 'text-ak-primary'
            }`}
          >
            {meetsCount}/{totalCount}
          </div>
          <div className="text-[11px] text-ak-text-secondary">Testkrav</div>
        </div>

        <div className="p-3 bg-ak-surface-subtle rounded-lg text-center">
          <div className={`text-2xl font-bold ${roundsMet ? 'text-ak-status-success' : 'text-ak-status-warning'}`}>
        ...
        <div className="p-3 bg-ak-surface-subtle rounded-lg text-center">
          <div className={`text-2xl font-bold ${handicapMet ? 'text-ak-status-success' : 'text-ak-status-error'}`}>
            {handicap.toFixed(1)}
          </div>
    ...
    <div className="w-full">
      ...
      {Object.entries(groupedRequirements).map(([category, reqs]) => (
        <div key={category} className="mb-6">
          <SubSectionTitle className="text-[15px] font-semibold text-ak-text-primary mb-3 flex items-center gap-2">
            {category}
            <span className="text-[12px] font-medium text-ak-text-secondary">
