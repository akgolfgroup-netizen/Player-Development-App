...
import {
  ...
  Flame, ChevronRight, Star, Filter, BarChart3
} from 'lucide-react';
...
const PYRAMID_LEVELS: PyramidLevel[] = [
  { id: 'ALL', label: 'Alle', color: 'var(--text-primary)', bgColor: 'bg-background-elevated' },
  { id: 'FYS', label: 'Fysisk', color: 'var(--error)', bgColor: 'bg-error/10' },
  { id: 'TEK', label: 'Teknikk', color: 'var(--ak-session-teknikk)', bgColor: 'bg-ak-session-teknikk/10' },
  { id: 'SLAG', label: 'Golfslag', color: 'var(--ak-session-golfslag)', bgColor: 'bg-ak-session-golfslag/10' },
  { id: 'SPILL', label: 'Spill', color: 'var(--success)', bgColor: 'bg-success/10' },
  { id: 'TURN', label: 'Turnering', color: 'var(--achievement)', bgColor: 'bg-achievement/10' },
];
...
const WeeklyHeatmap: React.FC<WeeklyHeatmapProps> = ({ data, pyramidLevels }) => {
  ...
  const getIntensity = (minutes: number): string => {
    ...
    if (ratio < 0.25) return 'var(--ak-primary-200)';
    if (ratio < 0.5) return 'var(--ak-primary-400)';
    if (ratio < 0.75) return 'var(--ak-primary-600)';
...
const ACWRGauge: React.FC<ACWRGaugeProps> = ({ data }) => {
  ...
  const getZoneColor = (ratio: number): string => {
    ...
    if (ratio <= 1.3) return 'var(--success)';
    if (ratio <= 1.5) return 'var(--warning)';
    return 'var(--error)';
  };
    ...
    <div className="flex flex-col items-center">
      ...
      <div className="flex gap-4 mt-2 text-xs">
        <span className="flex items-center gap-1">
          <span className="w-3 h-3 rounded-full bg-ak-primary/30 border border-ak-primary" />
          Din score
        </span>
        {dimensions.some(d => d.benchmark) && (
          <span className="flex items-center gap-1">
...
const PeriodizationTimeline: React.FC<PeriodizationTimelineProps> = ({
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        {[
          { type: 'base', label: 'Grunntrening', color: 'var(--ak-primary)' },
          { type: 'build', label: 'Oppbygging', color: 'var(--success)' },
          { type: 'peak', label: 'Toppform', color: 'var(--achievement)' },
          { type: 'taper', label: 'Nedtrapping', color: 'var(--warning)' },
          { type: 'competition', label: 'Konkurranse', color: 'var(--error)' },
...
const AreaCard: React.FC<AreaCardProps> = ({
  ...
  const pyramidColor = pyramidLevels.find(p => p.id === selectedPyramid)?.color || 'var(--ak-primary)';

  return (
    <Card className="hover:border-ak-primary/30 transition-colors cursor-pointer">
      <CardContent className="p-4">
...
const Treningsstatistikk: React.FC<TreningsstatistikkProps> = ({
  ...
  const periodPhases: PeriodPhase[] = [
    { id: '1', label: 'Grunntrening', startDate: '2025-10-01', endDate: '2025-11-30', type: 'base', color: 'var(--ak-primary)' },
    { id: '2', label: 'Oppbygging', startDate: '2025-12-01', endDate: '2026-01-15', type: 'build', color: 'var(--success)' },
    { id: '3', label: 'Toppform', startDate: '2026-01-16', endDate: '2026-02-10', type: 'peak', color: 'var(--achievement)' },
    { id: '4', label: 'Nedtrapping', startDate: '2026-02-11', endDate: '2026-02-17', type: 'taper', color: 'var(--warning)' },
    { id: '5', label: 'Konkurranse', startDate: '2026-02-18', endDate: '2026-02-23', type: 'competition', color: 'var(--error)' },
  ...
  const compareChartData = useMemo(() => {
    ...
    return areasToShow.map(area => ({
      ...
      color1: pyramid1Data?.color || 'var(--ak-primary)',
      color2: pyramid2Data?.color || 'var(--success)',
    }));
    ...
    <div className="min-h-screen bg-background-default">
      ...
      <div className="p-4 md:p-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <Card className="lg:col-span-2">
            ...
            <CardContent className="pb-8">
              <TrendLineChart data={trendData} height={140} color="var(--ak-primary)" />
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-semibold flex items-center gap-2">
              <Target size={16} className="text-ak-primary" />
