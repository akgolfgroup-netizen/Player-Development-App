    ...
    <div className="h-full flex flex-col">
      ...
      <div className="p-4 border-b border-ak-border-default">
        <div className="relative">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-ak-text-secondary" />
          <input
            type="text"
            placeholder="Søk økter..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full pl-10 pr-3 py-2 rounded-lg border border-ak-border-default text-sm outline-none focus:border-ak-primary"
          />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        {Object.entries(sessionsByCategory).map(([category, sessions]) => {
          ...
          return (
            <div key={category} className="border-b border-ak-border-default">
              {/* Category Header */}
              <button
                onClick={() => toggleCategory(category)}
                className="w-full flex items-center justify-between p-4 bg-ak-surface-subtle hover:bg-ak-border-default transition-colors"
              >
                ...
                <ChevronDown
                  size={18}
                  className={`text-ak-text-secondary transition-transform ${
                ...
                <div className="p-2 space-y-2">
                  {sessions.map((session) => (
                    <div
                      ...
                      className="p-3 rounded-lg bg-ak-surface-base border border-ak-border-default hover:border-ak-primary/50 hover:shadow-sm cursor-pointer transition-all group"
                    >
                      <div className="flex items-start justify-between mb-2">
                        <div className="flex-1">
                          ...
                          <p className="text-xs text-ak-text-secondary line-clamp-2">
                            {session.description}
                          </p>
                        </div>
                      </div>
      ...
      {/* Stats Footer */}
      <div className="p-4 border-t border-ak-border-default bg-ak-surface-subtle">
        <div className="text-xs text-ak-text-secondary text-center">
