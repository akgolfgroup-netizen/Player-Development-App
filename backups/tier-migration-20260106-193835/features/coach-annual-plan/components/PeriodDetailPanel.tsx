...
const PeriodDetailPanel: React.FC<PeriodDetailPanelProps> = ({
  ...
  return (
    <div className="fixed right-0 top-0 h-full w-96 bg-ak-surface-base border-l border-ak-border-default shadow-2xl z-50 flex flex-col">
      {/* Header */}
      <div className={`${config.bgColor} p-5 border-b border-ak-border-default`}>
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-lg bg-ak-surface-base/50 border-none flex items-center justify-center cursor-pointer hover:bg-ak-surface-base transition-colors"
          >
        ...
        <div className="flex gap-4 text-sm">
          <div>
            <div className="text-xs text-ak-text-tertiary mb-1">Varighet</div>
            <div className="font-semibold text-ak-text-primary">
              {duration.weeks} uker ({duration.days} dager)
            </div>
          </div>
          <div>
            <div className="text-xs text-ak-text-tertiary mb-1">Økter/uke</div>
            <div className="font-semibold text-ak-text-primary">
              ...
              <button
                ...
                className={`flex-1 py-2 rounded-lg text-sm font-medium cursor-pointer transition-all border ${
                  formData.weeklyFrequency === num
                    ...
                    : 'border-ak-border-default bg-ak-surface-base text-ak-text-primary hover:bg-ak-surface-subtle'
                }`}
              >
                {num}
              </button>
        ...
        <div className="mb-5">
          ...
          <div className="space-y-2 mb-3">
            {formData.goals.map((goal, index) => (
              <div
                key={index}
                className="flex items-start gap-2 p-3 rounded-lg bg-ak-surface-subtle border border-ak-border-default"
              >
                <Target size={16} className="text-ak-primary mt-0.5 flex-shrink-0" />
                <span className="flex-1 text-sm text-ak-text-primary">{goal}</span>
                <button
                  onClick={() => removeGoal(index)}
                  className="w-6 h-6 rounded-md bg-transparent border-none flex items-center justify-center cursor-pointer hover:bg-ak-status-error/10 transition-colors"
                >
                  <Trash2 size={14} className="text-ak-status-error" />
                </button>
              </div>
            ))}
          </div>
      ...
      {/* Footer Actions */}
      <div className="p-5 border-t border-ak-border-default space-y-2">
        <div className="flex gap-2">
          <Button
            variant="secondary"
            onClick={onClose}
        ...
        {onDelete && (
          <Button
            ...
            className="w-full text-ak-status-error hover:bg-ak-status-error/10"
          >
            Slett periode
          </Button>
        )}
